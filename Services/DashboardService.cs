    ...
    public class DashboardService : IDashboardService
    {
        ...
        public async Task<StandardResponse<DashboardView>> GetDashBoardMetrics()
        {
            try
            {
                ...
                var recentClients = _userRepository.ListUsers().Result.Users.Where(user => user.DateCreated <= DateTime.Now.AddMonths(1) && user.Role.ToLower() == "client").OrderByDescending(user => user.DateCreated).Take(5);
