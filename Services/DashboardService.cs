    ...
    public class DashboardService : IDashboardService
    {
        ...
        public async Task<StandardResponse<DashboardView>> GetDashBoardMetrics(Guid superAminId)
        {
            try
            {
                ...
                var recentPayrolls = _invoiceRepository.Query().Include(x => x.EmployeeInformation).Where(payroll => payroll.StatusId != (int)Statuses.PENDING && payroll.StatusId != (int)Statuses.INVOICED && payroll.EmployeeInformation.PayRollTypeId == 2 && payroll.EmployeeInformation.SuperAdminId == superAminId).ProjectTo<InvoiceView>(_configuration).OrderByDescending(payroll => payroll.DateCreated).Take(5);
                var recentInvoiced = _invoiceRepository.Query().Include(x => x.EmployeeInformation).Where(invoiced => invoiced.StatusId == (int)Statuses.INVOICED && invoiced.EmployeeInformation.SuperAdminId == superAminId).ProjectTo<InvoiceView>(_configuration).OrderByDescending(invoice => invoice.DateCreated).Take(5);
                var recentPayslips = _paySlipRepository.Query().Include(x => x.EmployeeInformation).Where(payslip => payslip.EmployeeInformation.SuperAdminId == superAminId).ProjectTo<PaySlipView>(_configuration).OrderByDescending(payslip => payslip.DateCreated).Take(5);
                var recentTimesheets = _timeSheetRepository.Query().Include(user => user.EmployeeInformation).ThenInclude(user => user.User).Where(timesheet => timesheet.Date < DateTime.Now && timesheet.EmployeeInformation.SuperAdminId == superAminId).OrderByDescending(timesheet => timesheet.Date).ProjectTo<TimeSheetView>(_configuration).Take(5);
