    ...
    public class DashboardService : IDashboardService
    {
        ...
        public async Task<StandardResponse<DashboardView>> GetDashBoardMetrics()
        {
            try
            {
                ...
                var allTeamMembers = _userRepository.Query().Include(x => x.EmployeeInformation).ThenInclude(x => x.Supervisor).Where(x => x.IsActive == true && (x.Role.ToLower() == "team member" || x.Role.ToLower() == "internal admin" || x.Role.ToLower() == "internal supervisor")).Take(5).OrderByDescending(x => x.DateCreated).ToList();
